    ...
    def read_artifact(arguments: dict[str, Any]) -> str:
        ...
        with _open_workspace(session) as ws:
            try:
                ...
                note = ""
                if arguments.get("run_id") and not arguments.get("hash"):
                    with contextlib.suppress(FoundationError):
                        note = _resolve_run(ws, str(arguments["run_id"]))[1]
                return f"{note}{e}"
        head = "\n".join(read.head)
        ...
        text = read.text
        lines = text.splitlines()
        if digested := _digest_unless_raw(arguments, read.name, text, len(lines)):
            return f"{head}\n{digested}"
    ...
    box.add(
        Tool(
            ...
            description=(
                ...
                "(or a hash prefix); run_id as for show_run. A run whose task was a "
                "cache hit holds no files for it: the read follows the hit to the run "
