...
to a *served* machine-learned interatomic potential for calculations
(training a new one is separate — the gracemaker builder, when this
machine configures it). Any canonical checkpoint id
...
If `list_engines` shows no checkpoint ids, this machine has no servable
MLIP — configure `[engines.rootstock]` first, or ask the site admin to
declare the checkpoint you need.
