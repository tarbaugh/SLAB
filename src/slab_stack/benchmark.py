...
def score_session(
    ...
    *session* names a Mason transcript or a harness session record. A
    ...
    if transcript is not None:
        ...
        # An external harness is an arm of its own, named by its client;
        # Mason's mechanisms were not in force, so none is listed.
        condition = str(harness.header().get("client") or "harness")
        ablated = []
        mechanisms = []
