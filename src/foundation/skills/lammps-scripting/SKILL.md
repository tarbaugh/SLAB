...
up and integrates no step, and the reply lists each LAMMPS error, the
checks, and the outputs. A check that failed is expected to, because no
step ran, and a check that passed on no data is not evidence. A check
that raised is a bug in the check: fix it before the real launch. It
costs one LAMMPS start and catches a syntax error in the third stage
and a wrong result key in the analysis before the MD leg is paid for. In a dry run
