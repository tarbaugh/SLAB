...
#: The keys under which a record carries setup lines: the composed block, and
#: the build's and the call's parts that the ``setup=`` provenance records.
_SETUP_KEYS = frozenset({"setup", "setup_build", "setup_call"})
...
def fold_setups(value: Any, shown_by: str) -> Any:
    ...
    Examples:
        ...
        >>> fold_setups({"setup": []}, "x")
        {'setup': []}
        >>> fold_setups({"provenance": {"setup_build": ["module load x"]}}, "x")["provenance"][
        ...     "setup_build"][:8]
        '1 lines '
    """
