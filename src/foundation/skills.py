...
beside optional ``scripts/``, ``references/``, and ``assets/``. Foundation
adds nothing to the format. One extension rides where the spec sends
extensions, the ``metadata`` map::

    metadata:
      mason-agents: "dft-expert analysis-expert"

names the resident agent's cards that see the skill (space-separated,
mirroring the spec's own ``allowed-tools`` convention). Absent means every
agent sees it. Foundation records the key on the :class:`Skill` and does
not interpret it: an external harness over MCP sees every skill. Frontmatter
keys this module does not know are ignored, so skills written for other
Agent Skills consumers load unmodified; the experimental ``allowed-tools``
field is accepted, ignored, and reported as ignored, because every consumer
gates its own execution.
...
3. built-in — shipped inside the ``foundation`` package, so the resident
   agent and an external harness load the same catalog

Progressive disclosure follows the spec: only ``name: description`` lines
enter a prompt or a catalog listing; the body loads when a client asks for
the skill by name; bundled files load only when the client reads or runs
them through its own primitives. A skill script therefore runs under
exactly the gate that governs every other command the client runs — there
...
class SkillError(FoundationError):
...
class Skill:
    ...
    def visible_to(self, agent_name: str) -> bool:
        """Whether the named resident-agent card sees this skill in its catalog."""
...
def _builtin_root() -> Path:
    return Path(str(importlib.resources.files("foundation"))) / "skills"
...
def bundled_files(skill: Skill) -> list[str]:
    """Every file under the skill root, as paths relative to it, hidden parts left out.
    ...
    listing (the same rule a search tool applies).
    """
    return sorted(
...
def listing(skill: Skill) -> str:
    """What a ``skill`` tool returns as text: the body, the root, the bundled files."""
    files = bundled_files(skill)
