...
whose package owns that table, so check `[agent]` with `slab mason doctor` and
`[workspace]` with `slab list`. Path values expand `~` and `${VAR}`,
...
Note what the launcher did not touch. A `slab run` payload is never prefixed
