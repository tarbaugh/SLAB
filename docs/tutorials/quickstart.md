...
| `slab runs reap` | Mark failed every run at status `running` whose recorded process on this host is gone. Mason does the same at session start and on every `list_runs` and `wait_for_run` call. |
