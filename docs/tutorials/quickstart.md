...
| `slab runs reap [--job ID]` | Mark failed every run at status `running` whose recorded process on this host is gone or whose scheduler job ended. With `--job`, fail that job's running runs without a liveness check. Mason does the same reap at session start and on every `list_runs` and `wait_for_run` call. |
