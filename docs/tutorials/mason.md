...
| `launch_workflow` | run a workflow script as a traced, check-gated run; this is how physics happens. `args` reach the script as argv; `background=true` detaches a long run so no tool timeout can touch it. `ntasks`, `threads`, and `gpus` size the run. The slice is reserved before the run starts, the run takes it as an affinity mask plus `CUDA_VISIBLE_DEVICES`, and a slice that does not fit what is free is refused with the free amounts. `dry_run=true` rehearses the script in a throwaway workspace, with LAMMPS set up and no step integrated, and records a `dry_run` event; each check comes back with what its outcome is worth, and `from_run` takes that run's cached results for every task call, so the checks run on real data; a real launch of a script text never dry-run in the session carries a warning line |
| `reverify_run` | run a script's checks again on a quarantined run's stored results, after a fix to a check that was wrong; no engine starts and no new run is recorded, the checks become a new verification pass, and the run moves to verified when all pass; a task whose inputs changed is refused, naming the task |
...
| `list_runs`, `show_run`, `list_engines` | the workspace's evidence surface: runs, checks with observed/expected values, failure records, capabilities; `list_runs` takes `session="this"` and `status="running"`, and first marks failed every running run whose recorded process on this host is gone; `show_run` folds finished tasks to one line each, `task=<label or seq>` returns one task's recipe, inputs, and outputs, and `full=true` returns them all; a run that is still going also lists its live files, the ones in its scratch directory that nothing has registered yet, and a `dry-<stamp>` id returns that dry-run record; `list_engines` also reports this host's `budget` and what is `free` right now, and each partition's declared fields, which are the caps a sized job is checked against |
| `free_resources` | what is free on this host right now: the budget, the free cpu and gpu counts, one line per gpu excluded after a refusal in this job, and one line per live reservation with its slice, its run or holder, and its age; the environment block's free amounts were read when the prompt was built, so call this before a concurrent launch; `list_runs` ends with the same free counts |
| `read_artifact` | one of a run's artifacts, by name or hash prefix; the way to read an engine's output file after the run. Digested first like `read_file`; `raw=true`, or `offset`/`limit`, gives the line-numbered text. The workflow script is kept as the run's `input` artifact under its own name. When a name is no artifact of the run, the tool looks in the run's scratch directory, so a running run's log reads the same way, headed as a live file and digested the same. The files of a run that died before it registered anything read the same way until the reap or the purge that follows removes its scratch. A `dry-<stamp>` id goes in `run_id` or `hash`, and without a `name` it lists the files that record holds |
...
The runs the job made, the session transcript it wrote, and the scratch
directories its calculations made under `[paths] scratch` belong to the
workspace. Purge sweeps them like any other session's. A job that was
cancelled or timed out leaves scratch behind, because the processes
inside died without cleaning up. Each directory carries the id of the
run that made it. `slab hpc cancel` removes it when it fails the job's
runs, and `slab purge` removes whatever is left once the run is over.
See [Lifecycle & retention](lifecycle-and-retention.md).
