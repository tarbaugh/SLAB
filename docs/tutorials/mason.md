...
| `read_artifact` | one of a run's artifacts, by name or hash prefix; the way to read an engine's output file after the run. Digested first like `read_file`; `raw=true`, or `offset`/`limit`, gives the line-numbered text. The workflow script is kept as the run's `input` artifact under its own name. A name that is no artifact of the run is looked for in the run's scratch directory, so a running run's log and the files of a run that died before it registered anything read the same way, headed as live files and digested the same. A `dry-<stamp>` id goes in `run_id` or `hash`, and without a `name` it lists the files that record holds |
