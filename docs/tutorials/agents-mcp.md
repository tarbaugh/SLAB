...
| `wait_for_run` | Block until a run finishes or the timeout passes. Takes an id, a prefix, or a run name. Without one, it returns when the first running run of this session finishes, names it in `run`, and lists the rest under `running`; `all=true` waits until none is running. `timeout_s` is 900 by default and capped at 21600 (6 hours), and `capped` says when the cap applied. A `still_running` answer carries `note_running`, which says that waiting again is the right call, and each entry carries `advance`: the time since the run started and the LAMMPS step against the end of the current `run`. A run whose recorded process on this host is gone is marked failed and answered at once with outcome `process_gone`. |
