...
def test_a_wait_records_only_the_run_that_just_finished(tmp_path: Path) -> None:
    ...
    assert "setup" not in engines[1] and engines[1]["setup_digest"] == engines[0]["setup_digest"]
    assert engines[1]["setup_lines"] == 1
...
def test_wait_results_name_the_finished_run_and_count_the_others(
    ...
    assert waited.startswith(f"run {second} (two): ") and first not in waited
