...
def test_the_doctor_says_when_the_plain_lammps_build_needs_a_gpu(
    ...
    # The launcher row is another check; here the binary is on PATH.
    monkeypatch.setattr(
        doctor,
        "launcher_after_setup",
        lambda command, setup: {"launcher": "lmp", "path": "/opt/lammps/bin/lmp", "detail": ""},
    )
    (project / "slab.toml").write_text(base + '[engines.lammps]\ncommand = "lmp"\n')
    result = runner.invoke(app, ["doctor", "--offline"])
    assert result.exit_code == 0, result.output
...
def test_the_doctor_counts_what_a_purge_would_sweep(
    ...
    """One row for the leftovers: scratch of dead processes, orphan
    transcripts, stale locks. A fact, not a failure, and a plus when clean."""
    ...
    clean = runner.invoke(app, ["doctor", "--offline"])
    assert clean.exit_code == 0, clean.output
    assert (
        "[+] leftovers: 0 scratch dir(s) (0 bytes), 0 orphan transcript(s), 0 stale lock(s)"
        in clean.output
    )

    ws = project / "ws"
    dead = seed_scratch(scratch_root, "slab-qe-dead", pid=vanished_pid())
    dead_bytes = sum(f.stat().st_size for f in dead.rglob("*") if f.is_file())
