...
from slab import EngineNotAvailableError, backends
...
def test_setup_probe_token_reads_rank_count_launcher_forms() -> None:
    ...
    # The command as written is the identity, so the versionless fingerprint
    # probes the template too: the unfilled rank count is stepped over.
    assert _setup_probe_token("mpirun -np {ntasks} pw.x") == "pw.x"
